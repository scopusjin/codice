...
def aggiorna_grafico():
    ...
    frase_secondaria_html = None  # eventuale variante

    # --- Helper numerico locale (se non già definito in alto) ---
    def _is_num(x):
    ...
    # --- Parametri aggiuntivi: traduzione in range (con traslazione oraria) ---
    ...
    # --- Determinazione Range Raffreddamento per Visualizzazione (grafico) ---
    ...
    # --- Range per intersezione complessiva ---
    ...
    # --- Stima minima Potente et al. (solo per gestione UI; non "stima cautelativa") ---
    mt_ore = None
    mt_giorni = None
    ...
    # Parametri aggiuntivi nei range d’intersezione (sempre limite inferiore; superiore se finito)
    for p in parametri_aggiuntivi_da_considerare:
        lo, hi = p["range_traslato"]
        if _is_num(lo):
            ranges_per_intersezione_inizio.append(lo)
            if _is_num(hi) and hi < INF_HOURS:
                ranges_per_intersezione_fine.append(hi)
            else:
                ranges_per_intersezione_fine.append(np.nan)
            nomi_parametri_usati_per_intersezione.append(p["nome"])

    # --- Logica di inclusione raffreddamento nell’intersezione ---
        ...
        if usa_potente_per_intersezione:
            ...
            if mt_ore is not None and not np.isnan(mt_ore):
                ...
                nomi_parametri_usati_per_intersezione.append(
                    "raffreddamento cadaverico (solo limite inferiore, affidabilità limitata)"
                )
        else:
            if t_med_raff_hensge_rounded_raw > 48:
                if altri_parametri_con_range:
                    if t_min_raff_hensge > 48:
                        ...
                        ranges_per_intersezione_fine.append(np.nan)
                        nomi_parametri_usati_per_intersezione.append("raffreddamento cadaverico (>48h)")
                    else:
                        ranges_per_intersezione_inizio.append(t_min_raff_hensge)
    ...
    # Potente non usato sopra ma disponibile → inserisci come parametro a parte
    if (not usa_potente_per_intersezione) and (mt_ore is not None) and (not np.isnan(mt_ore)):
        ranges_per_intersezione_inizio.append(mt_ore)
        ranges_per_intersezione_fine.append(np.nan)

    # --- Calcolo intersezione ---
    starts_clean = [s for s in ranges_per_intersezione_inizio if _is_num(s)]
    if not starts_clean:
        comune_inizio, comune_fine, overlap = np.nan, np.nan, False
    else:
        comune_inizio = max(starts_clean)
        superiori_finiti = [v for v in ranges_per_intersezione_fine if _is_num(v) and v < INF_HOURS]
    ...
    # --- Barre extra per grafico dai parametri aggiuntivi ---
    ...
    if num_params_grafico > 0:
        # compute_plot_data con fallback
        try:
            ...
            plot_data["extra_params"] = extra_params_for_plot  # compatibilità renderer legacy

        # clip intervalli extra al tail_end
    ...
    # --- NOTE/AVVISI: raccolta in 'avvisi' ---
    ...
    if hensge_input_forniti:
        ...
        if abs(Tr_val - T0_val) <= 1.0:
            avvisi.append(
                "Considerato che la temperatura rettale è molto simile alla temperatura ante-mortem stimata, "
                "è possibile che il raffreddamento si trovi ancora nella f
