...
def _render_desktop_cooling_label(text: str, help_text: str | None = None, help_key: str | None = None):
    """Etichetta desktop esterna al V2, con tooltip nativo Streamlit."""
...
with st.container(border=True):
    ...
    with col1:
        ...
        prev_rigor = st.session_state.get("selettore_macchie", livor_labels[0])
        if prev_rigor not in livor_labels:
            prev_rigor = livor_labels[0]
        scelta_macchie_lbl = st.selectbox(
            i18n.ui_text("full.livor_select_label"),
            options=livor_labels,
            index=livor_labels.index(prev_rigor),
        ...
        if stima_cautelativa_beta:
            if full_mobile:
                with st.container(gap="xsmall", key="cooling_prudent_v2_stack_mobile"):
                    ...
                    st.number_input(rectal_label, value=sget("rt_val", 35.0), step=0.1, format="%.1f", key="rt_val", label_visibility="collapsed")
                    st.number_input(i18n.ui_text("full.antemortem_temp_estimated_label"), value=sget("tm_val", 37.2), step=0.1, format="%.1f", key="tm_val", label_visibility="collapsed")
                    with st.container(horizontal=True, wrap=False, horizontal_alignment="distribute", vertical_alignment="center", gap="small", key="prudent_weight_row_mobile"):
                        with st.container(width="stretch", key="prudent_weight_value_mobile"):
                            st.number_input(i18n.ui_text("full.weight_label"), value=sget("peso", 70.0), step=1.0, format="%.1f", key="peso", label_visibility="collapsed")
                    ...
                    ta_base_val = st.number_input(i18n.ui_text("full.ta_base_input"), value=sget("ta_base_val", 20.0), step=0.1, format="%.1f", key="ta_base_val", label_visibility="collapsed")
                    ta_other_val = st.number_input(i18n.ui_text("full.ta_other_input"), value=sget("ta_other_val", ta_base_val), step=0.1, format="%.1f", key="ta_other_val", label_visibility="collapsed")
                    ta_values = [st.session_state.get("ta_base_val"), st.session_state.get("ta_other_val")]
                    ...
                    fc_min_val = st.number_input(i18n.ui_text("full.fc_min_input"), value=sget("fc_min_val", sget("fattore_correzione", 1.0)), step=0.1, format="%.2f", key="fc_min_val", label_visibility="collapsed")
                    fc_other_val = st.number_input(i18n.ui_text("full.fc_max_input"), value=sget("fc_other_val", sget("fattore_correzione", 1.0)), step=0.1, format="%.2f", key="fc_other_val", label_visibility="collapsed")
                    fc_values = [st.session_state.get("fc_min_val"), st.session_state.get("fc_other_val")]
                    ...
                    st.session_state["toggle_fattore"] = bool(st.session_state.get("toggle_fattore_inline", False))
            else:
                with st.container(gap="xsmall", key="cooling_prudent_v2_grid_desktop"):
                    ...
                    with top_left:
                        ...
                        with rt_col:
                            ...
                            st.number_input(i18n.ui_text("full.rectal_temp_label"), value=sget("rt_val", 35.0), step=0.1, format="%.1f", key="rt_val", label_visibility="collapsed", _mortem_compact_label="")
                        with tm_col:
                            _render_desktop_cooling_label("T. ante-mortem")
                            st.number_input(i18n.ui_text("full.antemortem_temp_estimated_label"), value=sget("tm_val", 37.2), step=0.1, format="%.1f", key="tm_val", label_visibility="collapsed", _mortem_compact_label="")
                    with top_right:
                        weight_col, weight_toggle_col = st.columns(2, gap="xsmall", vertical_alignment="bottom")
                        with weight_col:
                            _render_desktop_cooling_label("Peso")
                            st.number_input(i18n.ui_text("full.weight_label"), value=sget("peso", 70.0), step=1.0, format="%.1f", key="peso", label_visibility="collapsed", _mortem_compact_label="")
                        with weight_toggle_col:
                            st.toggle(i18n.ui_text("full.weight_uncertainty"), key="peso_stimato_beta")
                    _render_desktop_cooling_label("Range temperatura ambientale media", _TA_RANGE_DESKTOP_HELP, "ta_range")
                    ...
                    with ta_left:
                        ...
                        with ta_min_col:
                            ta_base_val = st.number_input(i18n.ui_text("full.ta_base_input"), value=sget("ta_base_val", 20.0), step=0.1, format="%.1f", key="ta_base_val", label_visibility="collapsed", _mortem_compact_label="")
                        with ta_max_col:
                            ta_other_val = st.number_input(i18n.ui_text("full.ta_other_input"), value=sget("ta_other_val", ta_base_val), step=0.1, format="%.1f", key="ta_other_val", label_visibility="collapsed", _mortem_compact_label="")
                    ta_values = [st.session_state.get("ta_base_val"), st.session_state.get("ta_other_val")]
                    if all(_is_num(v) for v in ta_values):
                        lo_ta, hi_ta = sorted(float(v) for v in ta_values)
                        st.session_state["Ta_min_beta"], st.session_state["Ta_max_beta"] = lo_ta, hi_ta
                        ...
                        st.session_state.pop("Ta_min_beta", None)
                        st.session_state.pop("Ta_max_beta", None)
                    _render_desktop_cooling_label("Range fattore di correzione (FC)", _FC_RANGE_DESKTOP_HELP, "fc_range")
                    ...
                    with fc_left:
                        ...
                        with fc_min_col:
                            fc_min_val = st.number_input(i18n.ui_text("full.fc_min_input"), value=sget("fc_min_val", sget("fattore_correzione", 1.0)), step=0.1, format="%.2f", key="fc_min_val", label_visibility="collapsed", _mortem_compact_label="")
                        with fc_max_col:
                            fc_other_val = st.number_input(i18n.ui_text("full.fc_max_input"), value=sget("fc_other_val", sget("fattore_correzione", 1.0)), step=0.1, format="%.2f", key="fc_other_val", label_visibility="collapsed", _mortem_compact_label="")
                    with fc_right:
                        suggest_col, _suggest_spacer = st.columns(2, gap="xsmall", vertical_alignment="bottom")
                        with suggest_col:
                            st.button("Consiglia", key="desktop_caut_fc_structural_suggest", type="secondary", width="stretch", on_click=_toggle_desktop_range_fc_suggest)
                    fc_values = [st.session_state.get("fc_min_val"), st.session_state.get("fc_other_val")]
                    if all(_is_num(v) for v in fc_values):
                        lo_fc, hi_fc = sorted(float(v) for v in fc_values)
                        st.session_state["FC_min_beta"], st.session_state["FC_max_beta"] = lo_fc, hi_fc
            ...
            if full_mobile:
                with st.container(gap="xsmall", key="cooling_standard_v2_stack_mobile"):
                    ...
                    st.number_input(i18n.ui_text("full.antemortem_temp_estimated_label"), value=sget("tm_val", 37.2), step=0.1, format="%.1f", key="tm_val", label_visibility="collapsed")
                    st.number_input(i18n.ui_text("full.weight_label"), value=sget("peso", 70.0), step=1.0, format="%.1f", key="peso", label_visibility="collapsed")
                    st.number_input(i18n.ui_text("full.ta_input_label"), value=sget("ta_base_val", 20.0), step=0.1, format="%.1f", key="ta_base_val", label_visibility="collapsed")
                    st.number_input(i18n.ui_text("full.fc_input_label"), value=sget("fattore_correzione", 1.0), step=0.1, format="%.2f", key="fattore_correzione", label_visibility="collapsed")
                ...
                with st.container(gap="xsmall", key="cooling_standard_v2_grid_desktop"):
                    ...
                    with top_right:
                        ...
                        with weight_col:
                            _render_desktop_cooling_label("Peso")
                            st.number_input(i18n.ui_text("full.weight_label"), value=sget("peso", 70.0), step=1.0, format="%.1f", key="peso", label_visibility="collapsed", _mortem_compact_label="")
                    _render_desktop_cooling_label("T. ambientale media", i18n.ui_text("full.ta_mean_help"), "ta_standard")
                    ta_left, _ta_right = st.columns(2, gap="small", vertical_alignment="bottom")
                    with ta_left:
                        ta_col, _ta_spacer = st.columns(2, gap="xsmall", vertical_alignment="bottom")
                        with ta_col:
                            st.number_input(i18n.ui_text("full.ta_input_label"), value=sget("ta_base_val", 20.0), step=0.1, format="%.1f", key="ta_base_val", label_visibility="collapsed", _mortem_compact_label="")
                    ...
                    with fc_left:
                        ...
                        with fc_col:
                            st.number_input(i18n.ui_text("full.fc_input_label"), value=sget("fattore_correzione", 1.0), step=0.1, format="%.2f", key="fattore_correzione", label_visibility="collapsed", _mortem_compact_label="")
                    with fc_right:
                        suggest_col, _suggest_spacer = st.columns(2, gap="xsmall", vertical_alignment="bottom")
                        with suggest_col:
                            st.button("Consiglia", key="desktop_caut_fc_structural_suggest", type="secondary", width="stretch", on_click=_toggle_desktop_single_fc_suggest)
                    st.session_state["toggle_fattore"] = bool(st.session_state.get("toggle_fattore_inline_std", False))

    if st.session_state.get("toggle_fattore", False):
        with st.container(border=False, key="full_fc_panel_mobile" if full_mobile else "full_fc_panel_desktop"):
            pannello_suggerisci_fc(peso_default=st.session_state.get("peso", 70.0), key_prefix="fcpanel_caut" if st.session_state.get("stima_cautelativa_beta", False) else "fcpanel_std")
...
if mostra_parametri_aggiuntivi:
    with st.container(border=True):
        ...
        if not usa_orario_custom_globale:
            st.markdown(i18n.ui_text("full.special_datetime_hint"), unsafe_allow_html=True)
        for parametro_id in FULL_SPECIAL_PARAM_BY_LABEL.values():
            ...
            with col2:
                selettore = st.selectbox(label=nome_parametro, options=list(full_special_option_labels(parametro_id)), key=f"{nome_parametro_legacy}_selector", label_visibility="collapsed")
            ...
            if selettore_id != OPTION_NOT_ASSESSED and usa_orario_custom_globale:
                ...
                with colx1:
                    st.markdown("<div style='font-size: 0.8em; color: orange; margin-bottom: 3px;'>" f"{i18n.ui_text('full.assessed_different_time')}" "</div>", unsafe_allow_html=True)
                with colx2:
            ...
            if usa_orario_custom_globale and usa_orario_personalizzato:
                ...
                with coly1:
                    ...
                    data_picker = st.date_input(measurement_date, value=input_data_rilievo, format="DD/MM/YYYY", key=f"{nome_parametro_legacy}_data", label_visibility="collapsed")
                with coly2:
                    ...
                    ora_value = (st.session_state.get(ora_key) or ora_main) if ora_manual else ora_main
                    ora_picker = native_time_picker(ora_value, key=f"{ora_key}_native", inherited=not ora_manual)
                    if ora_manual:
            ...
            widgets_parametri_aggiuntivi[nome_parametro_legacy] = {"selettore": selettore_legacy, "data_rilievo": data_picker, "ora_rilievo": ora_input}
...
def _inputs_signature():
    ...
    def _freeze(v):
        ...
        if isinstance(v, dict): return tuple(sorted((k, _freeze(val)) for k, val in v.items()))
        return str(v)
    base = [bool(st.session_state.get("usa_orario_custom", False)), bool(st.session_state.get("mostra_parametri_aggiuntivi", False)), bool(st.session_state.get("henssge_non_applicabile", False)), _freeze(st.session_state.get("input_data_rilievo")), _freeze(st.session_state.get("input_ora_rilievo")), _freeze(st.session_state.get("selettore_macchie") if "selettore_macchie" in st.session_state else None), _freeze(st.session_state.get("selettore_rigidita") if "selettore_rigidita" in st.session_state else None), _freeze(st.session_state.get("rt_val")), _freeze(st.session_state.get("ta_base_val") if "ta_base_val" in st.session_state else None), _freeze(st.session_state.get("tm_val")), _freeze(st.session_state.get("peso")), _freeze(st.session_state.get("fattore_correzione", 1.0)), bool(st.session_state.get("alterazioni_putrefattive", False)), bool(st.session_state.get("stima_cautelativa_beta", False))]
    try:
        from app.parameters import dati_parametri_aggiuntivi
        extra = []
        ...
        extra = []
    caut = [_freeze(st.session_state.get("Ta_min_beta")), _freeze(st.session_state.get("Ta_max_beta")), _freeze(st.session_state.get("FC_min_beta")), _freeze(st.session_state.get("FC_max_beta")), bool(st.session_state.get("peso_stimato_beta", False)), bool(st.session_state.get("range_unico_beta", False)), _freeze(st.session_state.get("ta_other_val")), _freeze(st.session_state.get("fc_other_val")), tuple(sorted(_freeze(st.session_state.get("fc_suggested_vals", []))))]
    return tuple(_freeze(base + extra + caut))

curr_sig = _inputs_signature()
if "last_run_sig" not in st.session_state: st.session_state["last_run_sig"] = None
if "show_results" not in st.session_state: st.session_state["show_results"] = False

st.markdown("""
    <style>
    div.stButton > button { border: 2px solid #2196F3 !important; color: black !important; background-color: white !important; font-weight: bold; border-radius: 8px !important; padding: 0.6em 2em !important; }
    div.stButton > button:hover { background-color: #E3F2FD !important; cursor: pointer; }
    html body:has(.mortem-full-title) [class*="st-key-desktop_caut_fc_structural_suggest"] button { box-sizing: border-box !important; width: 100% !important; height: 40px !important; min-height: 40px !important; max-height: 40px !important; margin: 0 !important; padding: 0.25rem 0.55rem !important; border: 1px solid color-mix(in srgb, var(--st-primary-color,#168AC1) 30%, transparent) !important; border-radius: 0.48rem !important; background: color-mix(in srgb, var(--st-primary-color,#168AC1) 14%, transparent) !important; color: var(--st-text-color,#31333F) !important; font-weight: 500 !important; box-shadow: none !important; }
    html body:has(.mortem-full-title) [class*="st-key-desktop_caut_fc_structural_suggest"] button:hover { background: color-mix(in srgb, var(--st-primary-color,#168AC1) 20%, transparent) !important; border-color: color-mix(in srgb, var(--st-primary-color,#168AC1) 45%, transparent) !important; }
    @media (min-width: 1024px) {
        html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] { box-sizing: border-box !important; width: min(100%, 82rem) !important; max-width: 82rem !important; margin-left: 0 !important; margin-right: auto !important; }
        html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] > [data-testid="stVerticalBlock"] { display: grid !important; position: relative !important; grid-template-columns: minmax(38rem, 2fr) minmax(19rem, 1fr) !important; grid-auto-flow: row !important; justify-content: start !important; column-gap: 1rem !important; row-gap: 0.30rem !important; align-items: start !important; }
        html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] > [data-testid="stVerticalBlock"] > * { grid-column: 1 !important; grid-row: auto !important; min-width: 0 !important; }
        html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] > [data-testid="stVerticalBlock"] > *:has(.mortem-full-title) { grid-column: 1 / -1 !important; grid-row: 1 !important; }
        html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] > [data-testid="stVerticalBlock"] > *:has([class*="st-key-btn_stima"]) { grid-column: 2 !important; grid-row: 3 !important; position: sticky !important; top: 1rem !important; width: 100% !important; max-width: 34rem !important; align-self: start !important; margin: 0 !important; z-index: 4 !important; }
        html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] > [data-testid="stVerticalBlock"] > *:has([class*="st-key-mortem_result_box"]), html body:has(.mortem-full-title):has([class*="st-key-stima_cautelativa_beta"]) [data-testid="stMainBlockContainer"] > [data-testid="stVerticalBlock"] > *:has([class*="st-key-mortem_no_data_box"]) { grid-column: 2 !important; grid-row: 4 / span 12 !important; position: sticky !important; top: 5.2rem !important; width: 100% !important; max-width: 34rem !important; align-self: start !important; margin: 0 !important; z-index: 3 !important; }
    }
    </style>
""", unsafe_allow_html=True)
...
if st.session_state["show_results"]:
    ...
    ha_parametro_aggiuntivo_stimabile = any(isinstance(dati_parametri_aggiuntivi.get(nome_parametro, {}).get("range", {}).get((widgets or {}).get("selettore")), tuple) for nome_parametro, widgets in widgets_parametri_aggiuntivi.items())
    if no_rt and no_macchie and no_rigidita and not ha_parametro_aggiuntivo_stimabile:
        ...
        st.stop()
    base_ok = (not st.session_state.get("henssge_non_applicabile", False) and not no_rt and input_ta is not None and input_tm is not None and input_w is not None and input_w > 0)
    prudente_ok = True
    if base_ok and st.session_state.get("stima_cautelativa_beta", False):
        ...
        if not prudente_ok: _warn_box(i18n.ui_text("full.henssge_incoherent_warning"))
    considera_raffreddamento = base_ok and (not st.session_state.get("stima_cautelativa_beta", False) or prudente_ok)
    aggiorna_grafico(selettore_macchie=selettore_macchie, selettore_rigidita=selettore_rigidita, input_rt=(input_rt if considera_raffreddamento else None), input_ta=(input_ta if considera_raffreddamento else None), input_tm=(input_tm if considera_raffreddamento else None), input_w=(input_w if considera_raffreddamento else None), fattore_correzione=st.session_state.get("fattore_correzione", 1.0), widgets_parametri_aggiuntivi=widgets_parametri_aggiuntivi, usa_orario_custom=st.session_state.get("usa_orario_custom", False), input_data_rilievo=st.session_state.get("input_data_rilievo"), input_ora_rilievo=st.session_state.get("input_ora_rilievo"), alterazioni_putrefattive=st.session_state.get("alterazioni_putrefattive", False), skip_warnings=True)

render_mobile_page_switch("Modalità sopralluogo", "pages/App_MSIL.py", "mobile_nav_footer_to_msil")
