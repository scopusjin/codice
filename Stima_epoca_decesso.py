    ...
    if henssge_non_app:
        ...
        if stima_cautelativa_beta:
            with st.container(key="prudent_explicit_ranges"):
                st.markdown(i18n.ui_text("full.prudent_default_note"), unsafe_allow_html=True)
            ...
            if full_mobile:
                with st.container(gap="xsmall", key="cooling_prudent_v2_stack_mobile"):
                    ...
                    if all(_is_num(v) for v in ta_values):
                        ...
                        st.session_state.pop("Ta_min_beta", None); st.session_state.pop("Ta_max_beta", None)
                    ...
                    if all(_is_num(v) for v in fc_values):
                        ...
                        st.session_state.pop("FC_min_beta", None); st.session_state.pop("FC_max_beta", None)
                ...
                with st.container(gap="small", key="cooling_prudent_v2_grid_desktop"):
                    ...
                    with c2:
                        st.number_input(i18n.ui_text("full.antemortem_temp_estimated_label"), value=sget("tm_val", 37.2), step=0.1, format="%.1f", key="tm_val", label_visibility="collapsed")
                    with st.container(horizontal=True, wrap=False, horizontal_alignment="distribute", vertical_alignment="center", gap="small", key="prudent_weight_row_desktop"):
    ...
    with st.container(border=True):
        ...
        for parametro_id in FULL_SPECIAL_PARAM_BY_LABEL.values():
            ...
            if not usa_orario_personalizzato:
                st.session_state.pop(ora_manual_key, None); st.session_state.pop(ora_last_main_key, None)
...
def _inputs_signature():
    ...
    try:
        ...
        for nome_parametro, _ in dati_parametri_aggiuntivi.items():
            extra.append(_freeze(st.session_state.get(f"{nome_parametro}_selector"))); extra.append(_freeze(st.session_state.get(f"{nome_parametro}_diversa"))); extra.append(_freeze(st.session_state.get(f"{nome_parametro}_data"))); extra.append(_freeze(st.session_state.get(f"{nome_parametro}_ora")))
...
if st.session_state["show_results"]:
    input_rt = st.session_state.get("rt_val"); input_ta = st.session_state.get("ta_base_val"); input_tm = st.session_state.get("tm_val"); input_w = st.session_state.get("peso")
    ...
    if no_rt and no_macchie and no_rigidita and not ha_parametro_aggiuntivo_stimabile:
        st.warning(i18n.ui_text("full.no_data_warning")); st.stop()
