...
/* radio group super-compatto */
div[data-testid="stRadio"]{margin:0!important;padding:0!important}
div[data-testid="stRadio"] > label{margin:0!important;padding:0!important}
div[data-testid="stRadio"] div[role="radiogroup"]{gap:.3rem!important;margin:0!important;padding:0!important}
...
with c1:
    input_w = st.number_input("Peso (kg)", value=st.session_state.get("peso", 70.0),
                              step=1.0, format="%.1f", label_visibility="visible", key="peso")
with c2:
    input_ta = st.number_input("T. ambientale media (°C)", value=st.session_state.get("ta_base_val", 20.0),
                               step=0.1, format="%.1f", label_visibility="visible", key="ta_base_val")
...
def pannello_suggerisci_fc_mobile(peso_default: float = 70.0, key_prefix: str = "fcpanel_m"):
    ...
    stato_corpo = st.radio("", ["Corpo asciutto", "Bagnato", "Immerso"],
    ...
    result = compute_factor(
        stato=("Asciutto" if stato_corpo == "Corpo asciutto" else stato_corpo),
        acqua=None, counts=counts,
