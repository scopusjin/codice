...
def calcola_fattore(peso: float):
    """
    UI + calcolo 'Fattore di correzione' con:
    - normalizzazione robusta delle etichette
    - alias bidirezionali per retro‑compatibilità (vestiti, coperte, superfici)
    - gestione Tabella 2 (correzione per peso) con ricerca più tollerante
    - stessa UX e stessi session_state key del resto dell'app
    """
    import unicodedata

    # ===== Caricamento tabelle (cached) + controlli base =====
    try:
    ...
    richieste_colonne = ["Ambiente", "Vestiti", "Coperte", "Superficie d'appoggio", "Correnti", "Fattore"]
    mancanti = [c for c in richieste_colonne if c not in tabella1.columns]
    if mancanti:
        st.error(f"Colonne mancanti in 'tabella rielaborata.xlsx': {', '.join(mancanti)}")
        return
    ...
    def _norm(s):
        ...
        # uniforma varianti di ">" al carattere U+02C3 (˃), gestendo anche doppie frecce
        s = (s.replace(">>", "˃˃")
               .replace("››", "˃˃")
               .replace("»»", "˃˃")
               .replace(">", "˃")
    ...
    # Alias (vecchie descrizioni -> nuove) e viceversa
    OLD_TO_NEW_UI = {
        # Coperte (vecchie etichette verbose -> nuove sintetiche)
    ...
    # Alias specifici VESTITI (gestione ˃ / ˃˃ e varianti)
    VESTITI_ALIASES = {
    ...
    def _aliases(val: str, colname: str):
        """Set di etichette equivalenti per ricerca in tabella."""
    ...
    LABEL_COPERTE = {
        ...
        # NB: se l'opzione UI è "Coperta" semplice, il format_func ricadrà su 'v' (ok).
    }
    ...
    corpo_asciutto = corpo_bagnato = corpo_immerso = False
    copertura_speciale = False
    scelta_vestiti = scelta_coperte = superficie = corrente = "/"
    ...
    # -- COL 2: COPERTE (solo se non bagnato/immerso)
    with col2:
    ...
    # -- COL 1: VESTITI (solo se asciutto/bagnato, niente per immerso o copertura speciale)
    ...
    # -- COL 2: CORRENTI D'ACQUA (solo se immerso)
    with col2:
    ...
    # -- COL 3: CORRENTI D'ARIA + SUPERFICIE
    with col3:
        # Correnti d'aria: visibili se non immerso, non copertura speciale; logica identica alla tua
    ...
    fattore_base = riga["Fattore"].iloc[0]
    try:
        fattore_base = float(fattore_base)
    except Exception:
    ...
    # ===== Tabella 2: correzione per peso =====
    # Applica solo quando serve (coerente con la tua logica)
    if fattore_base >= 1.4 and peso != 70:
        try:
            ...
            # Mappa colonne di Tabella 2 a pesi numerici
            ...
            # Trova la colonna più vicina a 70kg e quella più vicina al 'peso' selezionato
            ...
            # individua la riga con fattore più vicino a fattore_base nella colonna 70kg
    ...
    # ===== Bottone: applica fattore =====
    def _apply_fattore(val):
        st.session_state["fattore_correzione"] = round(float(val), 2)

        # Azzeriamo le stringhe descrittive (verranno ricostruite a valle)
